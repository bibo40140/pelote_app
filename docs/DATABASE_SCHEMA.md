...
#### `player_disciplines`
Inscription d'un joueur pour une discipline et une saison, avec sa série.
...
Contrainte UNIQUE partielle sur `(player_id, discipline_id, season_id)` lorsque `active = true`.

#### `teams`
Partenariat permanent historisé ; une nouvelle composition durable crée une nouvelle ligne.
...
Contrainte UNIQUE partielle sur `(team_id, player_id)` lorsque `active = true`. La cohérence avec la discipline et la saison de l'équipe dépend de `player_disciplines` actif.
...
| season_id | uuid | NOT NULL ; FK vers `seasons.id` |
| discipline_id | uuid | NOT NULL ; FK vers `disciplines.id` |
| series_id | uuid | NULL ; FK vers `series.id` |
| name | text | NOT NULL |
...
- `player_disciplines` relie un joueur, une discipline, une saison et une série.
- Une `team` appartient à une saison, une discipline et une série ; `team_members` en conserve les membres historiques.
...
- Une inscription active est unique pour un joueur, une discipline et une saison.
- Un membre actif d'équipe doit disposer d'une inscription active cohérente avec la discipline et la saison de l'équipe.
- Un joueur ne peut appartenir qu'à une équipe `ACTIVE` par discipline et saison. Cette règle doit inclure les membres actifs des équipes actives.
...
- Toute affectation doit concerner un joueur inscrit à la discipline et à la saison de l'événement.
...
- Le remplaçant doit être inscrit à la discipline et à la saison, disponible, distinct du joueur remplacé et sans conflit horaire. Toute dérogation est motivée et auditée.
...
- `player_disciplines` : index sur `(season_id, discipline_id, active)` et sur `(player_id, season_id, active)` ; index unique partiel de l'inscription active.
- `teams` : index sur `(season_id, discipline_id, status)` et `(series_id, status)` ; `team_members` sur `(player_id, active)`, `(team_id, active)` et un index unique partiel nécessaire à l'équipe active par joueur, discipline et saison.
...
- `competitions` : index `(season_id, discipline_id, status)` et `(starts_at)` ; `competition_teams` et `competition_matches` : index sur leurs clés étrangères de recherche.
...
3. **Création, régularisation ou archivage d'équipe** : vérifier les inscriptions, empêcher une seconde équipe active incompatible pour le joueur, contrôler le nombre de membres, mettre à jour le statut et conserver l'historique des membres avec audit.
...
5. **Validation d'un remplacement** : verrouiller la demande et l'affectation, confirmer que le demandeur est bien le joueur affecté, contrôler le remplaçant et les conflits, enregistrer le remplacement, modifier la représentation effective de l'affectation selon le modèle retenu, passer la demande à `APPROVED`, auditer et notifier atomiquement.
6. **Création ou modification d'une compétition et de ses équipes** : garantir en transaction la cohérence saison, discipline et série entre compétition et équipes engagées.
